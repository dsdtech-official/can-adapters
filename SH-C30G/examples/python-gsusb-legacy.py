...
"""Read and send CAN frames on an older SH-C30G from Python, on Windows.
...
10 kbit/s DOES NOT TRANSMIT ON THIS BOARD
    Measured on the SH-C30A, not on the SH-C30G itself -- same image,
...
ELSEWHERE = {
    (0x0100, "SH-C30x"):
        "the firmware a new SH-C30G ships with -- use python-gsusb.py",
    (0x0100, "sh-C30x"):
        "the firmware a new SH-C30G ships with -- use python-gsusb.py",
    (0x0000, "canable2 gs_usb"):
        "an SH-C31A, not an SH-C30G -- use that product's own examples",
    (0x0200, "SH-C31x"):
        "an SH-C31A, not an SH-C30G -- use that product's own examples",
}
