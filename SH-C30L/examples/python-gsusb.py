...
READ THIS FIRST -- ONE MODEL, TWO FIRMWARES, DIFFERENT BEHAVIOUR
    SH-C30L has shipped with two different firmwares over its life. Both are in
...
#       candleLight ecosystem ID. SH-C30L and SH-C31A both use it, and so do
...
#       TWO candidates (F01 on SH-C30L and F03 on SH-C31A) -- different chips,
...
HARDWARE_LIMITS = {
    ...
    # same class of transceiver. NOT measured on the SH-C30L itself. Treat the
...
def describe(entry):
    ...
    if entry["other_product"]:
        print("  ! this is not an SH-C30L: %s" % entry["other_product"])
        print("    %s" % ident)
    ...
    if fw is None:
        ...
        print("    either SH-C30L firmware. Identify it before trusting it.")
        return
...
def main() -> int:
    ...
    if not targets:
        print("no SH-C30L found among them")
        return 1
    if len(targets) > 1:
        print("more than one SH-C30L present; using the first. Select a specific")
