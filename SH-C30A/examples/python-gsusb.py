...
"""Read and send CAN frames on a DSD TECH SH-C30A from Python.

READ THIS FIRST -- ONE MODEL, TWO FIRMWARES, DIFFERENT BEHAVIOUR
    SH-C30A has shipped with two different firmwares over its life. Both are in
...
#       candleLight ecosystem ID. SH-C30A and SH-C31A both use it, and so do
...
#       TWO candidates (F01 on SH-C30A and F03 on SH-C31A) -- different chips,
...
def describe(entry):
    ...
    if entry["other_product"]:
        print("  ! this is not an SH-C30A: %s" % entry["other_product"])
        print("    %s" % ident)
    ...
    if fw is None:
        ...
        print("    either SH-C30A firmware. Identify it before trusting it.")
        return
...
def main() -> int:
    ...
    if not targets:
        print("no SH-C30A found among them")
        return 1
    if len(targets) > 1:
        print("more than one SH-C30A present; using the first. Select a specific")
